...
fn vscode_response(cmd: &str) -> Option<Value> {
    let copilot_perm = crate::core::config::Config::load()
        ...
        .unwrap_or_default();
    vscode_response_with_permission(
...
fn vscode_response_from_decision(decision: HookDecision, cmd: &str) -> Option<Value> {
    vscode_response_with_permission(decision, cmd, CopilotPermission::Omit)
}

/// `copilot_permission` opts a user back into asserting a decision on rewrites
/// that carry no explicit Allow rule: `Allow` emits "allow", `Ask` emits "ask",
/// `Omit` (the default) leaves the field out.
fn vscode_response_with_permission(
    decision: HookDecision,
    cmd: &str,
    copilot_perm: CopilotPermission,
) -> Option<Value> {
    ...
    let asserted = if allow {
        Some("allow")
    } else {
        match copilot_perm {
            CopilotPermission::Allow => Some("allow"),
            CopilotPermission::Ask => Some("ask"),
            CopilotPermission::Omit => None,
        }
    };
    if let Some(d) = asserted {
        hook_output["permissionDecision"] = json!(d);
    }
...
mod tests {
    ...
    #[test]
    fn test_vscode_copilot_permission_allow_asserts_allow() {
        let r = vscode_response_with_permission(
            HookDecision::AskRewrite("rtk cargo test".into()),
            "cargo test",
    ...
    #[test]
    fn test_vscode_copilot_permission_ask_asserts_ask() {
        let r = vscode_response_with_permission(
            HookDecision::AskRewrite("rtk cargo test".into()),
            "cargo test",
    ...
    #[test]
    fn test_vscode_copilot_permission_omit_leaves_field_out() {
        let r = vscode_response_with_permission(
            HookDecision::AskRewrite("rtk cargo test".into()),
            "cargo test",
    ...
    #[test]
    fn test_vscode_allow_rule_wins_over_copilot_permission_omit() {
        let r = vscode_response_with_permission(
            HookDecision::AllowRewrite("rtk git status".into()),
            "git status",
            CopilotPermission::Omit,
    ...
    #[test]
    fn test_vscode_deny_returns_none_regardless_of_copilot_permission() {
        assert!(vscode_response_with_permission(
            HookDecision::Deny,
            "cargo test",
            CopilotPermission::Allow
        )
